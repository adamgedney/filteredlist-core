  ...
  _createClass(_class, [{
    ...
    value: function run() {
      ...
      var state$ = this.rxdux.dispatch({
        ...
        _this2.hooks.onLoadingChange$.next({ loading: true, state: state });

        // Write our query to the url
        if (state.queryString) {
          _this2.queries._writeQueryStringToUrl(state.queryString, _this2.options);
        }

        // These _on hooks get picked up by the src/index file in the [initSubscriptions] fn
        if (filterObject.sort) {
          _this2.hooks._onSort$.next({ view: filterObject.view, sort: filterObject.sort, state: state });
        }
        if (filterObject.pagination) {
          _this2.hooks._onPaginationChange$.next({ view: filterObject.view, pagination: filterObject.pagination, state: state });
        }
        if (filterObject.filters) {
          _this2.hooks._onFilterChange$.next({ change: filterObject, state: state });
        }
      }));

      state$.subscribe(function () {}); // ensure a hook run
    ...
    value: function resetFilters() {
      ...
      var state$ = this.rxdux.dispatch({
        ...
        _this3.hooks._onFiltersReset$.next({ change: {}, state: state });
        _this3.hooks.onLoadingChange$.next({ loading: true });
      }));
      state$.subscribe(function () {});
  ...
  _createClass(_class, [{
    ...
    value: function setViews(views) {
      ...
      var views$ = this.rxdux.dispatch({
        ...
        _this.hooks.onViewsSet$.next({ views: _views });
      }));
    ...
    value: function selectView(id) {
      ...
      var selectedView$ = this.rxdux.dispatch({
        ...
        data: { id: id }
      }, 'selectedView').pipe((0, _operators.first)(), (0, _operators.tap)(function (selectedView) {
        _this2.hooks.onSelectedViewChange$.next({ selectedView: selectedView });
      }));
...
function _class() {
  ...
  this.onSelectedViewChange$ = new _rxjs.Subject();
  ...
  _createClass(_class, [{
    ...
    value: function dispatch() {
      ...
      if (selector && selector === 'state') {
        return (0, _rxjs.of)(this.store.getState());
      } else if (selector) {
